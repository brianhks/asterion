...
saw.setProperty(PomRule.GROUP_ID_PROPERTY, "org.asterion")
saw.setProperty(PomRule.URL_PROPERTY, "http://asterion.org")
...
pomRule = ivy.createPomRule("build/jar/pom.xml", ivy.getResolveRule("default"))
		...
		.addDeveloper("brianhks", "Brian", "brianhks1+asterion@gmail.com")
...
manifest.putValue("Implementation-Title", "Asterion")
...
scriptsFileSet = new RegExFileSet("src/scripts", ".*").addExcludeFile("asterion-env.sh")
...
tarRule = new TarRule("build/${programName}-${version}-${release}.tar")
		...
		.addFileTo(zipConfDir, "src/main/resources", "asterion.properties")
...
def doRPM(Rule rule)
{
	...
	rpmBuilder.with
			{
				...
				summary = summary
				type = RpmType.BINARY
				url = "http://code.google.com/p/asterion/"
	...
	rpmBuilder.addFile("/etc/init.d/asterion", new File("src/scripts/asterion-service.sh"), 0755)
	rpmBuilder.addFile("$rpmBaseInstallDir/conf/asterion.properties",
			new File("src/main/resources/asterion.properties"), 0644, new Directive(Directive.RPMFILE_CONFIG | Directive.RPMFILE_NOREPLACE))
	...
	rpmBuilder.addFile("$rpmBaseInstallDir/bin/asterion-env.sh",
			new File("src/scripts/asterion-env.sh"), 0755, new Directive(Directive.RPMFILE_CONFIG | Directive.RPMFILE_NOREPLACE))
...
//------------------------------------------------------------------------------
//Run the Asterion application
new SimpleRule("run-debug").setDescription("Runs asterion so a debugger can attach to port 5005")
		.addDepends(jp.getJarRule())
		.setMakeAction("doRun")
		.setProperty("DEBUG", true)

new SimpleRule("run").setDescription("Runs asterion")
		.addDepends(jp.getJarRule())
		.setMakeAction("doRun")
...
def doRun(Rule rule)
{
	...
	//Check if you have a custom asterion.properties file and load it.
	customProps = new File("asterion.properties")
	if (customProps.exists())
		args += " -p asterion.properties"
	...
	ret = saw.exec("java ${debug} -Dio.netty.epollBugWorkaround=true -cp ${runClasspath} org.asterion.Main ${args}", false)
	println(ret);
}
